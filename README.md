...
More information and documentation can be found in the the [kind-voltha README](https://github.com/opencord/kind-voltha/blob/master/README.md) which we recommend as the starting point
and the [voltha docs](https://docs.voltha.org/).
...
```
helm upgrade --install --create-namespace \
-n voltha voltha /home/ubuntu/khen/voltha-helm-charts/voltha-stack \
  --set global.stack_name=voltha \
  --set voltha_infra_name=voltha-infra \
  --set voltha_infra_namespace=infra \
  ...
  --set global.tracing.enabled=false \
  --set global.log_correlation.enabled=false 
```
